...
package com.ui
{
	public class DashedLine
	{
		 ...
		 * @return nothing
		 */
		public function lineStyle(thickness:Number,rgb:Number = 0,alpha:Number = 1):void {
		...
		private function lineLength(sx:Number, sy:Number, ex:Number = NaN, ey:Number = NaN):Number {
			if (isNaN(ex) && isNaN(ey)) return Math.sqrt(sx*sx + sy*sy);
			var dx:Number = ex - sx;
			var dy:Number = ey - sy;
			return Math.sqrt(dx*dx + dy*dy);
		}
		private function curveLength(sx:Number, sy:Number, cx:Number, cy:Number, ex:Number, ey:Number, accuracy:Number = NaN):Number {
			var total:Number = 0;
			var tx:Number = sx;
			var ty:Number = sy;
			var px:Number, py:Number, t:Number, it:Number, a:Number, b:Number, c:Number;
			var n:Number = (isNaN(accuracy) == false) ? accuracy : this._curveaccuracy;
